            ...
            try
            {
                ...
                if (usuarioValido)
                {
                    ...
                    if (user.EstaActivo)
                    {
                        ...
                        await HttpContext.SignInAsync(CookieAuthenticationDefaults.AuthenticationScheme, claimsPrincipal, new AuthenticationProperties { IsPersistent = true, ExpiresUtc = DateTime.UtcNow.AddMinutes(59) });
                    }
                    else
                    {
